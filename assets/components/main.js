...
if (switchToRegister) {
    console.log("Kaydolun baðlantýsý bulundu!");
} else {
    console.error("Kaydolun baðlantýsý bulunamadý!");
}

// Fonksiyon tanýmý
function snk_main_initializePage() {
    console.log("Sayfa baþlatýlýyor...");
...
// Blog yazýlarýný yükleme fonksiyonu
function snk_main_loadBlogPosts() {
    console.log("Blog yazýlarý yükleniyor...");
    // Bu fonksiyonun içeriðini doldurun
}

// Popup açma fonksiyonu
function snk_main_openCreatePostPopup() {
    console.log("Yeni blog yazýsý oluþturma popup'ý açýlýyor...");
    // Bu fonksiyonun içeriðini doldurun
}

// Sayfa yüklendiðinde çalýþacak fonksiyon
...
function snk_main_openCreatePostPopup() {
        ...
        <div class="snk-popup-overlay">
            <div class="snk-popup-container">
                <div class="snk-popup-header">
                    <h2>Yeni Blog Yazýsý Oluþtur</h2>
                ...
                <div class="snk-popup-content">
                    <form id="snk_createPostForm">
                        <div class="snk-form-group">
                            <label for="snk_postTitle">Baþlýk</label>
                            <input type="text" id="snk_postTitle" name="title" required>
                        </div>
                        <div class="snk-form-group">
                            <label for="snk_postContent">Ýçerik</label>
                        ...
                        <div class="snk-form-group">
                            ...
                            <select id="snk_postCategory" name="category">
                                ...
                                <option value="egitim">Eðitim</option>
                                <option value="yasam">Yaþam</option>
                                <option value="kultursanat">Kültür & Sanat</option>
                                <option value="bilim">Bilim</option>
                            </select>
                        </div>
                        <button type="submit" class="snk-submit-btn">Oluþtur</button>
                    </form>
                </div>
            </div>
        </div>
    ...
    createForm.addEventListener('submit', function (e) {
        ...
        const postData = {
            ...
            author: "Mustafa Demirsoy", // Bu kýsým dinamik olmalý
            status: "pending" // Onay bekliyor
        };

        // Yeni blog yazýsýný kaydet
...
function snk_main_saveBlogPost(postData) {
    console.log("Yeni blog yazýsý kaydediliyor:", postData);

    // Mevcut yazýlarý localStorage'dan al
    let existingPosts = JSON.parse(localStorage.getItem('snk_blog_posts') || '[]');

    // Yeni yazýya bir ID ata
    postData.id = existingPosts.length + 1;

    // Yeni yazýyý ekle
    existingPosts.push(postData);

    // localStorage'a kaydet
    localStorage.setItem('snk_blog_posts', JSON.stringify(existingPosts));

    // Sayfayý yenile veya yazýlarý yeniden yükle
    snk_main_loadBlogPosts();
}
