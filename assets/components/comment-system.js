/**
 * Yorum Sistemi JavaScript Dosyasý - Modüler Yapýda
 */

// Yorum sistemini kapsülleme - Global alaný kirletmemek için
const SNK_CommentSystem = (function () {
            ...
            <div class="snk-comment-container">
                ...
                <div class="snk-comment-form">
                    <div class="snk-comment-form-group">
                        <textarea class="snk-comment-textarea" id="commentText" placeholder="Düþüncelerinizi paylaþýn..."></textarea>
                    </div>
                    <div class="snk-comment-button-group">
                        <button class="snk-comment-button snk-comment-cancel" id="cancelComment">Ýptal</button>
                ...
                <div class="snk-comments-list" id="commentsList">
                    ...
                    <div class="snk-comment-empty" id="noCommentsMessage">
                        ...
                        <div class="snk-comment-empty-text">
                            Henüz yorum yok. Ýlk yorumu siz yapýn!
                        </div>
                    </div>
                </div>
            </div>
    ...
    function appendModalToBody() {
        // Modal HTML'i daha önce eklenmiþ mi kontrol et
    ...
    // Yorum sistemi butonlarýna olaylarý ekle
    ...
    function openModal(postId) {
        ...
        if (!currentUser) {
            alert('Yorum yapmak için giriþ yapmalýsýnýz!');
            return;
        }
    ...
    function submitNewComment() {
        // Aktif post kontrolü
        if (!activePostId) {
            console.error('Aktif post ID bulunamadý');
            return;
        }
        ...
        const comment = {
            ...
            userName: currentUser.name || currentUser.username || 'Kullanýcý',
            date: new Date().toISOString()
        };

        // Yanýt verme kontrolü
        ...
        // Yanýt verme bilgilerini temizle
        cancelReply();

        // Yorumlarý yeniden yükle
        loadComments(activePostId);

        // Baþarýlý bildirim göster
        showNotification('Yorumunuz baþarýyla eklendi!');
    }

    // Bildirim gösterme
    function showNotification(message) {
        // Mevcut bildirim varsa kaldýr
        ...
        // Animasyon için sýnýf ekle
        setTimeout(() => {
            notification.classList.add('show');
        }, 10);

        // 3 saniye sonra bildirimi kaldýr
        setTimeout(() => {
    ...
    function loadComments(postId) {
        if (!postId) {
            console.error("loadComments: Post ID bulunamadý!", postId);
            return;
        }
        ...
        // Ýlgili post yorumlarýný filtrele
        const comments = allComments.filter(comment => comment.postId === postId);
        console.log("Bu blog yazýsýnýn yorumlarý:", comments);

        // Yorumlarý göster
        displayComments(comments);
    }

    // Yorumlarý görüntüleme
    function displayComments(comments) {
        ...
        // Yorum varsa mesajý gizle
        if (noCommentsMessage) {
            noCommentsMessage.style.display = 'none';
        }

        // Yorumlarý tarihe göre sýrala (en yeni üstte)
        ...
        comments.forEach(comment => {
            ...
            const replyContent = comment.replyTo ? `
                <div class="snk-comment-reply-to">
                    ...
                    <span>${comment.replyToUserName || 'Kullanýcý'}</span>'a yanýt:
                </div>` : '';
        ...
        // Yorum silme butonlarýna olay ekle
    ...
    function cancelReply() {
        ...
        if (commentText) {
            ...
            // Textarea içeriðini temizle
            if (commentText.value.startsWith('@')) {
                commentText.value = '';
            }

            // Yanýt göstergesini gizle
    ...
    function deleteComment(commentId) {
        if (confirm('Bu yorumu silmek istediðinizden emin misiniz?')) {
            // Tüm yorumlarý al
            const allComments = getAllComments();
    ...
    function sharePost(postId, event) {
        // Týklama olayýnýn yayýlmasýný önle
        if (event) {
        ...
        // Mevcut paylaþým paneli varsa kaldýr
                    ...
                    navigator.clipboard.writeText(shareUrl)
                        .then(() => {
                            ...
                            if (copyBtn) {
                                ...
                                copyBtn.querySelector('.snk-share-option-text').textContent = 'Kopyalandý!';
        ...
        // Hemen document click olayýný devre dýþý býrakalým 
        // Bunu yapmak için bir overlay ekleyeceðiz
        ...
        // Panel açýldýðýnda týklama olayýnýn dýþarý yayýlmasýný önle
        panel.addEventListener('click', function (e) {
            e.stopPropagation();
        });

        // Görsel efekt için sýnýf ekle (animasyon için)
        setTimeout(() => {
            panel.classList.add('active');
            overlay.classList.add('active');
        }, 10);

        // Kapatma butonuna týklandýðýnda kapat
        const closeButton = panel.querySelector('.snk-share-panel-close');
        if (closeButton) {
            closeButton.addEventListener('click', function (e) {
                e.stopPropagation(); // Týklama olayýnýn yayýlmasýný önle
                panel.classList.add('closing');
                overlay.classList.add('closing');
                setTimeout(() => {
                    panel.remove();
        ...
        // URL kopyalama butonuna týklandýðýnda kopyala
        ...
        // Paylaþým seçeneklerine týklama olaylarý ekle
    ...
    function getCurrentUser() {
        // Ana sisteme uygun þekilde kullanýcý bilgisini al
        // Ýlk olarak snk_currentUser'ý kontrol et
        let user = JSON.parse(localStorage.getItem('snk_currentUser') || '{}');

        // Eðer kullanýcý bulunduysa ve giriþ yapmýþsa
        if (user && user.isLoggedIn) {
            return user;
        }

        // Alternatif olarak blogUser'ý kontrol et
    ...
    // Ýlklendirme
    function init() {
        // Modal'ý body'ye ekle
        appendModalToBody();

        // Butonlarý ayarla - sayfa tamamen yüklendiðinde
        document.addEventListener('DOMContentLoaded', function () {
            setupCommentButtons();
        });

        // Sayfa yüklendiyse ve DOMContentLoaded olayý tetiklendiyse
...
// Yorum sistemini otomatik baþlat
SNK_CommentSystem.init();

// Sayfa yüklendiðinde yorum sistemi butonlarýný tekrar ayarla
window.addEventListener('load', function () {
    // 500ms bekledikten sonra butonlarý tekrar ayarla (dinamik içerik için)
    setTimeout(function () {
        SNK_CommentSystem.init();
    }, 500);
});
