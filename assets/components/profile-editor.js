 ...
 * Profil düzenleme sayfasý için JavaScript iþlevselliði
 */
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    // Sayfa yüklendiðinde localStorage'dan verileri yükle
    loadProfileData();

    /**
     * Profil Düzenleme Modal'ýný Göster
     */
    function showProfileEditModal() {
        if (profileEditOverlay) {
            profileEditOverlay.classList.add('active');
            document.body.style.overflow = 'hidden'; // Sayfayý kaydýrmayý engelle
        }
    }

    /**
     * Profil Düzenleme Modal'ýný Gizle
     */
    function hideProfileEditModal() {
        if (profileEditOverlay) {
            profileEditOverlay.classList.remove('active');
            document.body.style.overflow = ''; // Sayfayý kaydýrmayý tekrar etkinleþtir
        }
    }
    ...
    /**
     * Form alanlarýný mevcut profil verilerine göre doldur
     */
    function populateFormFields() {
        ...
        if (savedProfileData) {
            ...
            // Eðer localStorage'da veri yoksa, DOM'dan mevcut bilgileri topla
            const nameInput = document.getElementById('profileName');
            const displayNameInput = document.getElementById('profileDisplayName');
            const bioInput = document.getElementById('profileBio');
     ...
     * @param {Event} event - Dosya seçme olayý
    ...
    function previewImage(event, previewElement) {
        ...
        if (file) {
            ...
            reader.onload = function (e) {
                ...
                // Resim elementini oluþtur veya güncelle
     ...
     * @param {Event} e - Form submit olayý
     */
    async function handleProfileFormSubmit(e) {
        ...
        try {
            ...
            // FormData'dan JavaScript nesnesine dönüþtür
            for (const [key, value] of formData.entries()) {
                profileData[key] = value;
            }

            // Profil ve kapak resimleri için özel iþlem
            ...
            // Eðer resim yoksa direkt localStorage'a kaydet
            if ((!profilePictureInput || profilePictureInput.files.length === 0) &&
            ...
            // Baþarýlý güncelleme mesajý
            alert('Profiliniz baþarýyla güncellendi!');

            // Modal'ý kapat
            hideProfileEditModal();

        } catch (error) {
            console.error('Profil güncelleme hatasý:', error);
            alert('Profil güncellenirken bir hata oluþtu. Lütfen tekrar deneyin.');
        }
    }
    ...
    function updateProfileUI(profileData) {
        ...
        // Ana profil alanlarýný güncelle
        ...
        // Hakkýnda sekmesindeki kiþisel bilgileri güncelle
        updateAboutSectionPersonalInfo(profileData);

        // Hakkýnda sekmesindeki sosyal medya bilgilerini güncelle
    ...
    /**
     * Sayfa yüklendiðinde localStorage'dan profil verilerini yükler ve UI'a uygular
     */
    ...
    if (profileEditBtn) {
        profileEditBtn.addEventListener('click', function () {
            ...
            populateFormFields(); // Modal açýldýðýnda form alanlarýný doldur
        });
    }
    ...
    // Modal dýþýna týklandýðýnda kapat
    if (profileEditOverlay) {
    ...
    // ESC tuþuna basma olayýný dinle
