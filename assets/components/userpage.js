// Kullanýcý profil sayfasý JavaScript iþlevselliði
document.addEventListener('DOMContentLoaded', function () {
    console.log('Userpage JS loaded');

    // Sekme deðiþtirme iþlevselliði - Temel iþlevsellik
    function initializeTabs() {
        ...
        if (userTabs.length > 0 && userSections.length > 0) {
            ...
            // Ýlk yüklemede animasyonlarý baþlat
            animatePosts();
        }
    }

    // Takip et butonu iþlevselliði
    ...
    // Mesaj gönder butonu iþlevselliði
    function initializeMessageButton() {
        const messageBtn = document.getElementById('user_messageBtn');
        if (messageBtn) {
            messageBtn.addEventListener('click', function () {
                alert('Mesaj özelliði yakýnda eklenecek!');
            });
        }
    }

    // Yazý kartlarý için animasyonlar
    ...
    // Rozet kartlarý için animasyonlar
    ...
    function initializeDarkMode() {
        ...
        if (darkModeToggle) {
            ...
            if (checkBox) {
                // Mevcut duruma göre checkbox'ý ayarla
